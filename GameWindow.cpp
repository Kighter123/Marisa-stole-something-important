...
GameWindow::GameWindow(QWidget *parent)
    ...
    horiObstacleCount(2),//初始侧面障碍物
    horiSpeed(4.0f),//初始横向障碍物速度
    score(0),//初始分数
    rewardBlock(1200, 800)//奖励函数初始状态
{
    ...
    for (int i = 0; i < obstacleCount; ++i) {
        obstacles.append(new Obstacle(this, QRandomGenerator::global()->bounded(1000), -50 - i * 200));
    }
}
...
void GameWindow::paintEvent(QPaintEvent *event) {
    ...
    painter.drawText(width() - 200, 50, QString("Score: %1").arg(score));
    painter.setPen(Qt::black);
    painter.setFont(QFont("Arial", 24));
    painter.drawText(width() - 200, 100, QString("life: %1").arg(player->getLife()));
}
...
void GameWindow::updateGame() {
    ...
    // 随时间增加顶部障碍物
    if (elapsedTime >= 4.0f && obstacleCount < 80) {  // 画面中上限 80 个
        //基于随机的x，并预留一定的y，
        obstacles.append(new Obstacle(this, QRandomGenerator::global()->bounded(1200), -30));
        obstacleCount+=3;
    ...
    // 检测玩家与奖励方块的碰撞
    if (rewardBlock.checkCollision(player->rect())) {
        score += 50;  // 增加50分
        player->activateInvincibility();
        update();     // 触发重绘
    }
